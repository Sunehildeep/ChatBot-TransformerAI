...
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads):
        ...
        self.d_model = d_model  # embedding vector length
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, maximum_position_encoding, rate=0.1):
        ...
        # Mind the repetitions of encoder layer :-)
    ...
    def call(self, x, training, mask):
        ...
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))  # This is done in the research I don't have the proper intuition, ignore
        x += self.pos_encoding[:, :seq_len, :]
        x = self.dropout(x, training=training)
        for i in range(self.num_layers):  x = self.enc_layers[i](x, training, mask)
